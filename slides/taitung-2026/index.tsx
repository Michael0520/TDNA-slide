...
export const notes: (string | undefined)[] = [
  ...
  `【中文】\n第三個，Claude Code，今天的主角。它的官方文件在 code.claude.com，有繁體中文版。\n安裝超簡單，在 Warp 裡貼一行 curl 指令就好。\n裝完之後，在 Warp 打 claude 就能啟動它。等下我們整堂課都會用它。\n\n———\n\n【English】\nThird, Claude Code, today's star. The docs are at code.claude.com, and there's a Chinese version.\nInstalling is very easy — just paste one curl line in Warp.\nWhen it's done, type claude in Warp to start it. We'll use it for the rest of the day.`,
  `【中文】\n來，一起檢查裝好了沒。在 Warp 裡一個一個打：node -v、pnpm -v、git --version、claude --version。\n每一個都應該跳出一個版本號碼 —— 只要看到版本號，就代表那個工具裝好了。\n四個都看到版本的，舉手讓我確認一下。\n（這頁多留一點時間，這是第一個關卡，沒過的個別去救援，確保大家都過了再往下）\n\n———\n\n【English】\nLet's check together. In Warp, type these one by one: node -v, pnpm -v, git --version, claude --version.\nEach one should show a version number. If you see a number, that tool is installed.\nRaise your hand if you see all four.\n(give this extra time — it's the first checkpoint; help anyone stuck, and make sure everyone is ready before moving on)`,
  `【中文】\n工具到位了。第一段我們把環境跟觀念都準備好了。\n接下來第二段，我們學怎麼真正「驅動」Claude Code，還有一個讓 AI 從「能用」變成「可靠」的關鍵習慣，叫 SDD。\n\n———\n\n【English】\nThe tools are ready. In hour one, we set up the environment and the ideas.\nNext, in hour two, we learn how to "drive" Claude Code, plus one habit that makes AI reliable: SDD.`,
  // H2
  `【中文】\n歡迎回來。\n工具大家都裝好了，現在進到重點：怎麼「驅動」它 —— 也就是怎麼讓 Claude Code 真的照你的意思做事，而不是亂做一通。\n\n———\n\n【English】\nWelcome back. Everyone has the tools now. Now the main part: how to "drive" it.\nThat means: how to make Claude Code do what you want, not just do random things.`,
  `【中文】\n打開 Claude Code 真的很簡單：在 Warp 裡打一個字 claude，按 enter，它就啟動了。\n然後你就用平常講話的方式，跟它說你要什麼就好 —— 不用背任何指令、不用記語法。\n這就是它對新手最友善的地方。\n\n———\n\n【English】\nOpening Claude Code is really easy. In Warp, type the word claude, press enter, and it starts.\nThen you just talk to it in normal words and tell it what you want — no commands to remember, no special rules.\nThat's why it's so easy for beginners.`,
  `【中文】\n這裡我要特別講一個習慣，它是 vibe coding 跟 AI coding 的分水嶺 —— Plan Mode。\n在 Claude Code 裡連按兩次 Shift+Tab，它就不會馬上動手，而是先給你一份計畫：它打算做什麼、要改哪些檔案、按什麼順序。\n你看過、覺得 OK、按核可，它才開始做。\n核心一句話：動手前先給計畫，不要邊做邊發明。\n（這句停一下，讓它沉下去）這就是你拿回主導權的關鍵 —— AI 想偏了，你在計畫階段就攔下來，不用等它做完一團才後悔。\n\n———\n\n【English】\nLet me show you one habit. It's the line between Vibe Coding and AI Coding: Plan Mode.\nIn Claude Code, press Shift+Tab twice. It won't act right away — first it gives you a plan: what it will do, which files it will change, and in what order.\nYou read it, you say OK, and then it runs.\nThe key line: plan first, then act. Don't make things up as you go. (pause, let it land)\nThis is how you stay in control — if the AI goes the wrong way, you stop it at the plan, not after a big mess.`,
  `【中文】\n為什麼要這麼麻煩？因為如果你只是用嘴巴一直聊，通常會踩三個坑。\n第一，風格不一致 —— 你今天講的跟明天講的不一樣，做出來就不一樣。\n第二，漏掉狀況 —— AI 只做了正常情況，沒網路、欄位空白這些它沒想到。\n第三，最危險的，假性自信 —— AI 會很開心地跟你說「做完了！」但你要問：做完什麼？\n\n———\n\n【English】\nWhy do this? Because if you just chat, you usually hit three problems.\nOne, it's not the same each time — what you say today is different from tomorrow, so the result is different.\nTwo, it misses cases — the AI only does the normal path; no internet, empty fields, it forgets these.\nThree, the worst one, fake confidence — the AI happily says "Done!" — but done with what?`,
  ...
  `【中文】\n裝好就用用看。\n跟 Claude Code 說「用 /polish 幫我的個人站打磨設計」。\n為什麼需要它？因為 AI 看過太多類似的範本，如果你沒特別講，它就會生出那種「一看就知道是 AI 做的」設計 —— Inter 字體、紫藍漸層、卡片疊卡片。\nImpeccable 做的事，就是把「好設計」變成 AI 跟你之間共用的詞彙，讓它知道你要的是什麼。\n\n———\n\n【English】\nNow use it. Tell Claude Code: "use /polish to improve my site's design."\nWhy do you need it? The AI has seen too many templates. If you don't say anything, it makes that obvious "AI look" — Inter font, purple-blue gradient, cards on cards.\nImpeccable gives the AI and you the same design words, so it knows what you really want.`,
  `【中文】\n三小時走到這了，我們回顧一下你今天帶走了什麼。\n第一小時，你把工具都裝好了，也懂了 Vibe 跟 AI Coding 的差別。\n第二小時，你會用 Claude Code 了，也學會用 SDD 把「要什麼」講清楚。\n第三小時，你從一個範本，一路做到一個 live URL，真的 ship 了自己的網站。\n回家之後，就用今天這套工作方式，把你下一個想法也 ship 出來。\n\n———\n\n【English】\nThree hours done — let's look at what you take home.\nHour one, you installed the tools and learned Vibe vs AI Coding.\nHour two, you can drive Claude Code, and you learned to say clearly what you want with SDD.\nHour three, you went from a template to a live URL — you shipped your own site.\nAt home, use this same way to ship your next idea.`,
  `【中文】\n謝謝大家今天的耐心，三個小時對講者跟學員都不短。\n兩件事帶走：一個是你今天親手 ship 的網址，另一個是一整套用 AI 把東西做出來的工作方式。\n有任何問題、卡關、想多聊的，都來找我，我會留到 17:30，michaello.me 跟 GitHub 都歡迎。\n\n———\n\n【English】\nThank you all for being here — three hours is long for everyone.\nTwo things to take home: the URL you shipped today, and a way to build things with AI.\nAny questions, anything you're stuck on — come find me. I'll be here until 17:30. michaello.me and GitHub are both welcome.`,
];
